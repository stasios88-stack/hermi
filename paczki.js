(function() {
    ...
    let rozmiar = parseInt(localStorage.getItem('gemini_paczka_size')) || 200;
    ...
    if (!document.getElementById('gemini-monitor')) {
        ...
        setInterval(() => {
            ...
            if (btn && !btn.dataset.hooked) {
                ...
                setBtn.innerText = `USTAW PACZKĘ (${rozmiar})`;
                setBtn.className = 'btn';
                setBtn.style.marginLeft = '5px';
                setBtn.onclick = (e) => {
                    e.preventDefault();
                    let v = prompt("Rozmiar paczki:", rozmiar);
                    if (v) { localStorage.setItem('gemini_paczka_size', v); location.reload(); }
                };
                btn.after(setBtn);
